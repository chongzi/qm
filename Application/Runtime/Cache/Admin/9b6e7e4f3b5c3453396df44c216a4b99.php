				...
				<div class="content">
					<table class="no-border blue">
						...
						<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							<td style="width:50%;"><?php echo ($vo['username']); ?></td>
							<td style="width:20%;"><?php echo (long2ip($vo["last_login_ip"])); ?></td>
							<td style="width:20%;"><?php echo (time_format($vo["last_login_time"],"Y-m-d")); ?></td>
							<td class="text-right">
								<a data-placement="left" data-toggle="tooltip" data-original-title="修改" class="label label-primary" href="<?php echo U('password?id='.$vo['id']);?>"><i class="fa fa-pencil"></i></a>
								<?php if($vo['id'] != 1): ?><a data-placement="left" data-toggle="tooltip" data-original-title="禁用" class="label <?php if(($vo["IsActive"]) == "0"): ?>label-default<?php else: ?>label-warning<?php endif; ?> confirm no-refresh ajax-get status" callback="status(<?php echo ($i); ?>)" href="<?php echo U('status?id='.$vo['id']);?>"><i class="fa fa-ban"></i></a><?php endif; ?>
							</td>
